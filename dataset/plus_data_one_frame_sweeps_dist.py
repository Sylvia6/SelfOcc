...
class Plus_One_Frame_Sweeps_Dist:
    def __init__(
            self, 
            data_path="/mnt/bigfile_2/lu.li/plus_occ_data", 
            imageset='train', 
            num_cams = 2,
            crop_size=[540, 640],
        ...
        # load data
        bag_pathes = os.listdir(self.data_path)
        self.keyframes = []
        self.scene_infos = dict()
        for bag_path in bag_pathes:
            path = os.path.join(self.data_path, bag_path)
            img_frames = self.load_data(path)
            infos = []
            index = 0
            for i in range(0, len(img_frames), self.num_cams):
                frames = img_frames[i: i + self.num_cams]
                info = {}
                info["bagname"] = bag_path
                info['token'] = info["bagname"]  # NOTE(mingyao.li) use bagname as token for now
                info["timestamp"] = frames[0]["timestamp"]    # stereo
                info["n_imgs"] = self.num_cams
                info["cam2imu"] = np.array(frames[0]["c2imu"])
                info["imu2world"] = np.array(frames[0]["imu2w_vio"])
                info["cam2pixel"] = np.array([f["intr"] for f in frames])
                info["data"] = dict()
                for f in frames:
                    f["c2w_vio"] = np.array(f["imu2w_vio"]) @ np.array(f["c2imu"])
                    info["data"][f["cam_name"]] = f
                infos.append(info)
                self.keyframes.append((bag_path, index))
                index += 1
            self.scene_infos.update({bag_path: infos})
        ...
        for scene_idx, (scene_token, scene_samples) in enumerate(self.scene_infos.items()):
            ...
            for i, sample in enumerate(scene_samples):
                curr_xyz = get_xyz(sample['data'][ref_sensor]['c2w_vio'])
                prev_samples, next_samples = [], []
                prev_dists, next_dists = [], []
                for j in range(i - 1, -1, -1):
                    temp_xyz = get_xyz(scene_samples[j]['data'][ref_sensor]['c2w_vio'])
    ...
    def load_data(self, data_path):
        data_file = os.path.join(data_path, "transforms.json")
    ...
    def get_data_info(self, info):
        # NOTE(mingyao.li) mind that some bag does not have, we need to fix this in the future 
        image_paths = []
        lidar2img_rts = []
        img2lidar_rts = []
        cam_intrinsics = []
        ...
        lidar2ego = np.array(info['data'][self.ref_sensor]['l2imu'])
        ego2lidar = np.linalg.inv(lidar2ego) # TODO(mingyao.li) repace to safe inverse
        lidar2global = info['imu2world'] @ lidar2ego
...
if __name__ == '__main__':
    import torch
    dataset = Plus_One_Frame_Sweeps_Dist(
        "/mnt/bigfile_2/lu.li/plus_occ_data",
