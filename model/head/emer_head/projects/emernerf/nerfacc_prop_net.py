...
class ProposalNetworkSampler(torch.nn.Module):
    def __init__(self, proposal_estimator, 
                 proposal_networks,
                 num_samples,
        ...
        super().__init__()
        self.proposal_estimator=proposal_estimator
    ...
    def is_required_grad(self):
        return self.proposal_requires_grad_fn(self._step)
    ...
    def update_every_n_steps(
        ...
        self.proposal_estimator.update_every_n_steps(
            trans, requires_grad, loss_scaler
        )

    def forward(self, ray_bundle: RayBundle, proposal_requires_grad)->RaySamples:
        ...
        t_starts, t_ends = self.proposal_estimator.sampling(
            prop_sigma_fns=[
        ...
        # print("t_starts shape=", t_starts.shape)
        # print("t_ends shape=", t_ends.shape)
        # print("origins shape=", chunk_data_dict[prefix + "origins"].shape)
        # print("viewdirs shape=", chunk_data_dict[prefix + "viewdirs"].shape)
        frustums = Frustums(
            origins=chunk_data_dict[prefix + "origins"][..., None, :],  # [..., 1, 3]
            directions=chunk_data_dict[prefix + "viewdirs"][..., None, :],  # [..., 1, 3]
            starts=t_starts[..., None],  # [..., num_samples, 1]
            ends=t_ends[..., None],  # [..., num_samples, 1]
            pixel_area=ray_bundle.pixel_area[..., None, :],  # [..., 1, 1]
        )
        # positions = t_origins + t_dirs * (t_starts + t_ends)[..., None] / 2.0

        ray_samples = RaySamples(
            frustums=frustums,
            camera_indices=ray_bundle.camera_indices[..., None],  # [..., 1, 1]
            deltas=None,  # [..., num_samples, 1]
            spacing_starts=None,  # [..., num_samples, 1]
            spacing_ends=None,  # [..., num_samples, 1]
            spacing_to_euclidean_fn=None,
            metadata=None,
            times=None if ray_bundle.times is None else ray_bundle.times[..., None],  # [..., 1, 1]
        )
        return ray_samples
